...
ETL Pipeline: HomeHarvest listed-vs-sold housing data -> Snowflake star schema

Reads for_sales.csv and sold.csv, cleans and conforms them, and produces
warehouse-ready CSVs for a star schema:
    FACT_HOME_TRANSACTION  +  DIM_PROPERTY  DIM_LOCATION  DIM_DATE
...
# ----------------------------------------------------------------------
# 1. EXTRACT  -- read both source files, tag listed vs sold, combine
# ----------------------------------------------------------------------
def extract():
    for_sales = pd.read_csv("for_sales.csv")
    sold      = pd.read_csv("sold.csv")

    for_sales["transaction_type"] = "Listed"   # active / pending listings
    sold["transaction_type"]      = "Sold"     # completed / expired sales
...
# ----------------------------------------------------------------------
# 2. TRANSFORM  -- clean, type-cast, derive columns, standardize county
# ----------------------------------------------------------------------
def transform(df):
    # Standardize MLS status casing
    ...
    # Numeric coercion (bad values -> NaN)
    ...
    # Drop rows missing the keys we must have
    before = len(df)
...
# ----------------------------------------------------------------------
# 3. BUILD STAR SCHEMA  -- one fact + three dimension tables
# ----------------------------------------------------------------------
def build_star_schema(df):
    # --- DIM_LOCATION : one row per county/city/zip -------------------
    ...
    # --- DIM_PROPERTY : one row per physical property -----------------
    ...
    # --- DIM_DATE : one row per date that appears anywhere ------------
    ...
    # --- FACT_HOME_TRANSACTION : one row per listing/sale ------------
    fact = df.copy()
    # attach location_key
    fact = fact.merge(
    ...
    # attach date_key (based on list_date)
...
# ----------------------------------------------------------------------
# 4. LOAD  -- write each table to CSV for Snowflake bulk load
# ----------------------------------------------------------------------
